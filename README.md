...
- **Multi-provider LLM layer (sift-robust).** Anthropic primary (Sonnet 4.6 drafter, Haiku 4.5 classifier); OpenAI fallback (gpt-4o drafter, gpt-4o-mini classifier) when `OPENAI_API_KEY` is set. Fallback fires on rate-limit / overload / connection errors only — not on auth/config errors. Every triage response surfaces `classification_provider` and `drafting_provider` so silent provider drift is impossible. The faithfulness scorer (`app/faithfulness.py`) is deliberately Anthropic-only to keep eval comparability stable across runs. **Privacy note:** when the OpenAI fallback fires, your ticket text is sent to OpenAI's API in addition to Anthropic. Forkers running on real customer data should evaluate this against their compliance posture; the fallback can be disabled by leaving `OPENAI_API_KEY` unset.
